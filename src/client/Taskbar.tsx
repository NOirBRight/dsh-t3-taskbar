...
export function Taskbar(props: Props) {
  ...
  const onTogglePin = (sessionId: string, isPinned: boolean) => {
    runCommand(isPinned ? { type: 'Unpin', sessionId } : { type: 'Pin', sessionId })
  }
  const onWake = (sessionId: string) => {
    runCommand({ type: 'Wake', sessionId })
  }
  const onSnooze = (sessionId: string, until: number) => {
    const pending = list.byId[sessionId as never]?.pendingInteraction
    runCommand(pending === undefined
  ...
  const renderSlim = (card: Card) => {
    ...
    return (
      <div key={card.sessionId} className={`dsht3-row dsht3-slim${card.selected ? ' dsht3-on' : ''}`}>
        ...
        <button type="button" className="dsht3-wake" onClick={() => onWake(card.sessionId)}>
          {t('wake')}
        </button>
  ...
  const renderCard = (card: Card) => {
    ...
    if (card.slim === true) return renderSlim(card)
  ...
  const empty = pinned.length === 0 && active.length === 0 && snoozed.length === 0 && unsentDrafts.length === 0
        ...
        {searching ? (
          ...
          <>
            ...
            {snoozed.length === 0 ? null : (
              <section className="dsht3-shelf">
                <button type="button" className="dsht3-shead" onClick={() => setSnoozedOpen((was) => !was)}>
                  {t('shelf.snoozed')}
                </button>
                {snoozedOpen ? snoozed.map(renderSlim) : null}
              </section>
            )}
          </>
        )}
          ...
          <div className="dsht3-menu" style={{ left: menu.x, top: menu.y }} onClick={(event) => event.stopPropagation()}>
            {snoozePanel ? (
              ...
              <>
                ...
                {snoozedIds.has(menu.id) || list.byId[menu.id as never]?.pendingInteraction !== undefined ? null : (
